...
class MessageQuerySet(models.query.QuerySet):
    ...
    def get_conversation(self, sender, recipient):
        """用户间的私信会话"""
        qs_one = self.filter(sender=sender, recipient=recipient).select_related('sender', 'recipient')  # A 发送给 B 的消息
        qs_two = self.filter(sender=recipient, recipient=sender).select_related('sender', 'recipient')  # B 发送给 A 的消息
    ...
    def get_most_recent_conversation(self, recipient):
        ...
        :param recipient: 传入的用户对象, 登录用户
        :return: 最近一次私信互动的用户 或者 自己
        """
        try:
